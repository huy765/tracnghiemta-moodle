...
use mod_rquiz\privacy\provider;

defined('MOODLE_INTERNAL') || die();

/**
 * Class mod_rquiz_privacy_provider_testcase
 */
class mod_rquiz_privacy_provider_testcase extends \core_privacy\tests\provider_testcase {
    ...
    protected function setUp(): void {
        ...
        // Create 2 quizzes.
        /** @var mod_rquiz_generator $plugingen */
        $plugingen = $gen->get_plugin_generator('mod_rquiz');
        $params = [
        ...
        foreach ($this->quizzes as $quiz) {
            ...
            foreach ($questions as $qtext => $answers) {
                ...
                $qins->id = $DB->insert_record('rquiz_question', $qins);
                $qins->answers = [];
                ...
                foreach ($answers as $atext) {
                    ...
                    $ains->id = $DB->insert_record('rquiz_answer', $ains);
                    $qins->answers[] = $ains;
                }
                $quiz->questions[] = $qins;
                $qnum++;
        ...
        $DB->insert_record('rquiz_submitted', $ins);
        $ins = (object)[
    ...
    public function test_get_metadata(): void {
        $collection = new collection('mod_rquiz');
        $newcollection = provider::get_metadata($collection);
        $itemcollection = $newcollection->get_collection();
        $this->assertCount(1, $itemcollection);

        $table = array_shift($itemcollection);
        $this->assertEquals('rquiz_submitted', $table->get_name());
        ...
        $this->assertEquals('privacy:metadata:rquiz_submitted', $table->get_summary());

        // Make sure all language strings exist.
        foreach ($privacyfields as $langstr) {
            get_string($langstr, 'mod_rquiz');
        }
        get_string($table->get_summary(), 'mod_rquiz');
    }
    ...
    public function test_get_contexts_for_userid(): void {
        $cms = [
            get_coursemodule_from_instance('rquiz', $this->quizzes[0]->id),
            get_coursemodule_from_instance('rquiz', $this->quizzes[1]->id),
        ];
        $expectedctxs = [
    ...
    public function test_export_for_context(): void {
        ...
        $this->export_context_data_for_user($this->student->id, $ctxs[0], 'mod_rquiz');
        $writer = \core_privacy\local\request\writer::with_context($ctxs[0]);
        $this->assertTrue($writer->has_any_data());

        $this->export_context_data_for_user($this->student->id, $ctxs[1], 'mod_rquiz');
    ...
    public function test_delete_data_for_all_users_in_context(): void {
        ...
        $this->assertEquals(5, $DB->count_records('rquiz_submitted', []));

        // Delete data from the first quiz.
        $cm = get_coursemodule_from_instance('rquiz', $this->quizzes[0]->id);
        $cmcontext = context_module::instance($cm->id);
        provider::delete_data_for_all_users_in_context($cmcontext);
        // After deletion, there should be 2 submitted responses.
        $this->assertEquals(2, $DB->count_records('rquiz_submitted', []));

        // Delete data from the second quiz.
        $cm = get_coursemodule_from_instance('rquiz', $this->quizzes[1]->id);
        $cmcontext = context_module::instance($cm->id);
        provider::delete_data_for_all_users_in_context($cmcontext);
        // After deletion, there should be 0 submitted responses.
        $this->assertEquals(0, $DB->count_records('rquiz_submitted', []));
    }
    ...
    public function test_delete_data_for_user(): void {
        ...
        $session2->id = $DB->insert_record('rquiz_session', $session);
        $ins = (object)[
            'questionid' => $this->quizzes[1]->questions[0]->id,
        ...
        $this->assertEquals(6, $DB->count_records('rquiz_submitted', []));

        // Delete the data for the first student, for the first quiz.
        $contextlist = new \core_privacy\local\request\approved_contextlist($this->student, 'rquiz',
                                                                            [$ctxs[0]->id]);
        provider::delete_data_for_user($contextlist);

        // After deletion, we should have 3 submitted responses.
        $this->assertEquals(3, $DB->count_records('rquiz_submitted', []));
        // Confirm the remaining responses are for the second student.
        $this->assertEquals([$student->id], $DB->get_fieldset_select('rquiz_submitted', 'DISTINCT userid', "1=1"));

        // Delete the data for the second student, for all quizzes.
        $contextlist = new \core_privacy\local\request\approved_contextlist($student, 'rquiz',
                                                                            [
                                                                                $ctxs[0]->id, $ctxs[1]->id
                                                                            ]);
        provider::delete_data_for_user($contextlist);
    ...
    public function test_get_users_in_context(): void {
        ...
        $userlist = new \core_privacy\local\request\userlist($ctxs[0], 'mod_rquiz');
        provider::get_users_in_context($userlist);
        $this->assertCount(1, $userlist);

        $userlist = new \core_privacy\local\request\userlist($ctxs[1], 'mod_rquiz');
        provider::get_users_in_context($userlist);
    ...
    public function test_delete_data_for_users(): void {
        ...
        $userlist = new \core_privacy\local\request\userlist($ctxs[0], 'mod_rquiz');
        provider::get_users_in_context($userlist);
        $approvedlist = new \core_privacy\local\request\approved_userlist($ctxs[0], 'mod_rquiz', [$this->student->id]);
